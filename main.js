/* ACE2X Knowledge OS v0.5.0 - generated by esbuild. Source: src/main.ts */
  ...
  onOpen() {
    ...
    apply.onclick = async () => {
      ...
      try {
        ...
        console.error(error);
        new import_obsidian.Notice(`Metadata sync failed: ${error.message || error}`);
...
var ACE2XKnowledgeOSPlugin = class extends import_obsidian.Plugin {
  async onload() {
    ...
    this.registerEvent(this.app.vault.on("rename", async (file, oldPath) => {
      ...
      if (changed) {
        await this.persist();
        new import_obsidian.Notice("Knowledge OS folder settings updated after folder rename.");
      }
    }));
  ...
  vaultFolderPaths() {
    return this.app.vault.getAllLoadedFiles().filter((entry) => entry instanceof import_obsidian.TFolder).map((folder) => (0, import_obsidian.normalizePath)(folder.path)).filter(Boolean).sort((a, b) => a.localeCompare(b, void 0, { numeric: true, sensitivity: "base" }));
  }
  updateConfiguredPathsAfterFolderRename(oldPath, newPath) {
    oldPath = (0, import_obsidian.normalizePath)(oldPath || "");
    newPath = (0, import_obsidian.normalizePath)(newPath || "");
    let changed = false;
    for (const key of this.folderSettingKeys()) {
      const current = (0, import_obsidian.normalizePath)(this.settings[key] || "");
      if (current === oldPath || current.startsWith(oldPath + "/")) {
        this.settings[key] = (0, import_obsidian.normalizePath)(newPath + current.slice(oldPath.length));
        changed = true;
      }
    }
    return changed;
  ...
  async autoDetectFolders() {
    ...
    for (const [key, terms] of Object.entries(mappings)) {
      const current = (0, import_obsidian.normalizePath)(this.settings[key] || "");
      const currentExists = current && this.app.vault.getAbstractFileByPath(current) instanceof import_obsidian.TFolder;
  ...
  isManagedRecordPath(file) {
    ...
    return Object.values(RECORD_TYPES).some((definition) => {
      const folder = (0, import_obsidian.normalizePath)(this.settings[definition.folderSetting] || "");
  ...
  shouldProcess(file) {
    if (!(file instanceof import_obsidian.TFile) || file.extension !== "md") return false;
    const path = (0, import_obsidian.normalizePath)(file.path);
    const peopleFolder = (0, import_obsidian.normalizePath)(this.settings.peopleFolder);
  ...
  async previewCurrentNote() {
    ...
    if (!file) return new import_obsidian.Notice("No active note.");
    if (!this.shouldProcess(file)) return new import_obsidian.Notice("This note is excluded from knowledge sync.");
  ...
  async previewCurrentFolder() {
    ...
    if (!active) return new import_obsidian.Notice("No active note.");
  ...
  async previewBatch(title, files) {
    if (!files.length) return new import_obsidian.Notice("No eligible notes found.");
    const analyses = [];
  ...
  async undoLastTransaction() {
    if (!this.lastTransaction) return new import_obsidian.Notice("Nothing to undo.");
    ...
    this.lastTransaction = null;
    new import_obsidian.Notice(`Undo complete: restored ${count} files.`);
  }
  ...
  async syncFile(file, showNotice = false) {
    ...
    const result = await this.applyAnalysis(analysis);
    await this.persist();
    if (showNotice) new import_obsidian.Notice(`Synced ${result.decisions} decisions, ${result.risks} risks, ${result.issues} issues and ${result.executive} executive follow-ups.`);
  }
  ...
  isPersonFile(file) {
    ...
    const folder = (0, import_obsidian.normalizePath)(this.settings.peopleFolder);
  ...
  resolvePersonLink(link, sourceFile) {
    ...
    if (!(destination instanceof import_obsidian.TFile) || !this.isPersonFile(destination)) return null;
    return {
  ...
  async createPersonFile(target) {
    ...
    const path = (0, import_obsidian.normalizePath)(`${folder}/${displayName}.md`);
    if (this.app.vault.getAbstractFileByPath(path)) return;
    let content = "---\ntype: person\naliases:\n  - {{name}}\n---\n\n# {{name}}\n";
    const template = this.app.vault.getAbstractFileByPath((0, import_obsidian.normalizePath)(this.settings.personTemplatePath));
    if (template instanceof import_obsidian.TFile) content = await this.app.vault.cachedRead(template);
  ...
  recordFolder(type) {
    ...
    return (0, import_obsidian.normalizePath)(this.settings[definition.folderSetting]);
  }
  yamlQuote(value) {
  ...
  async syncRecordNotes(records, sourceFile, sourceLink, date) {
    ...
    for (const record of records) {
      ...
      await this.ensureFolder(folder);
      const path = (0, import_obsidian.normalizePath)(`${folder}/${this.recordFileName(record, date)}`);
      expected.add(path);
    ...
    for (const definition of Object.values(RECORD_TYPES)) {
      ...
      for (const candidate of this.app.vault.getMarkdownFiles()) {
        ...
        const fm = (_a = this.app.metadataCache.getFileCache(candidate)) == null ? void 0 : _a.frontmatter;
  ...
  async ensureKnowledgeBase() {
    const folder = (0, import_obsidian.normalizePath)(this.settings.dashboardFolder || "01.Home/Dashboards");
    await this.ensureFolder(folder);
    ...
    const path = (0, import_obsidian.normalizePath)(`${folder}/${baseName}.base`);
    if (this.app.vault.getAbstractFileByPath(path)) return;
  ...
  async syncKnowledgeOSStatusChanges() {
    ...
    if (!recordFiles.length) {
      new import_obsidian.Notice("No Knowledge OS records were found.");
      return;
    }
    let changed = 0;
    let failed = 0;
    ...
    if (failed) {
      new import_obsidian.Notice(`Knowledge OS status sync completed: ${changed} changed, ${failed} failed.`);
    } else if (changed) {
      new import_obsidian.Notice(`Knowledge OS status sync completed: ${changed} source item${changed === 1 ? "" : "s"} updated.`);
    } else {
      new import_obsidian.Notice("Knowledge OS status sync completed. No status changes were found.");
    }
  }
  ...
  async syncRecordStatusToSource(recordFile) {
    ...
    if (!(sourceFile instanceof import_obsidian.TFile) || !this.shouldProcess(sourceFile)) return false;
...
var ACE2XKnowledgeOSSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Automatic sync").setDesc("Process notes after saving. Manual preview is recommended.").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoSync).onChange(async (value) => {
      this.plugin.settings.autoSync = value;
      await this.plugin.persist();
    }));
    const folderPaths = this.plugin.vaultFolderPaths();
    new import_obsidian.Setting(containerEl).setName("Auto-detect folders").setDesc("Find likely People, Executive, Decisions, Risks, Issues and Dashboard folders when a configured location is missing.").addButton((button) => button.setButtonText("Detect folders").onClick(async () => {
    ...
    new import_obsidian.Setting(containerEl).setName("Dashboard Base name").setDesc("Filename for the master Base. Enter a name without .base. Changing it creates the new Base on the next sync and does not delete the previous file.").addText((text) => text.setPlaceholder("00.\u{1F39B}\uFE0F Master").setValue(this.plugin.settings.dashboardBaseName || "00.\u{1F39B}\uFE0F Master").onChange(async (value) => {
      this.plugin.settings.dashboardBaseName = value.trim().replace(/\.base$/i, "") || "00.\u{1F39B}\uFE0F Master";
      await this.plugin.persist();
    }));
    new import_obsidian.Setting(containerEl).setName("Create unresolved links as people").setDesc("Off by default. When enabled, unresolved links inside d::, r::, i:: or e:: records create person pages.").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoCreatePeople).onChange(async (value) => {
      this.plugin.settings.autoCreatePeople = value;
      await this.plugin.persist();
    }));
    new import_obsidian.Setting(containerEl).setName("Excluded folders").setDesc("Comma-separated vault paths.").addTextArea((text) => text.setValue((this.plugin.settings.excludedFolders || []).join(", ")).onChange(async (value) => {
      this.plugin.settings.excludedFolders = value.split(",").map((x) => (0, import_obsidian.normalizePath)(x.trim())).filter(Boolean);
      await this.plugin.persist();
    }));
    new import_obsidian.Setting(containerEl).setName("Excluded headings").setDesc("Comma-separated heading prefixes ignored during analysis.").addTextArea((text) => text.setValue((this.plugin.settings.excludedHeadings || []).join(", ")).onChange(async (value) => {
  ...
  addFolderPicker(containerEl, name, description, settingKey, folderPaths) {
    const current = (0, import_obsidian.normalizePath)(this.plugin.settings[settingKey] || "");
    const exists = current && this.app.vault.getAbstractFileByPath(current) instanceof import_obsidian.TFolder;
    const setting = new import_obsidian.Setting(containerEl).setName(name).setDesc(exists ? description : `${description} Current location is missing; select a folder or run auto-detect.`);
    setting.addDropdown((dropdown) => {
      ...
      dropdown.onChange(async (value) => {
        this.plugin.settings[settingKey] = (0, import_obsidian.normalizePath)(value);
        await this.plugin.persist();
        this.display();
      });
    });
